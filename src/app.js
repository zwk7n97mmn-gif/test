...
import { AUTOEDIT_DEFAULTS, buildCutPlan, editSummary, fullClip, layoutClips, sanitizeClips, splitAt, timelineDuration, timelineToSource } from './core/autoedit.js';
import { buildAudioPlan, describeAudioPlan } from './core/audio.js';
import { createPersistence, createProject, createStore, emptyAnalysis } from './core/project.js';
import { normalizeCues } from './core/subtitles.js';
...
const subtitleEditor = createSubtitleEditor({
  store,
  onSeek: (sourceTime) => {
    const { timelineTime } = sourceToTimelineSafe(sourceTime);
    seek(timelineTime);
  },
...
const thumbnailEditor = createThumbnailEditor({
  store,
  getThumbVideo: () => media.thumbVideo,
...
function renderMediaPanel(extra) {
  ...
  if (!state.media) {
    children.push(
      emptyState(
        '素材が読み込まれていません',
        '動画ファイル（MP4 / WebM / MOV）を選択するか、画面左のエリアへドラッグ＆ドロップしてください。',
        button('動画ファイルを選択', { variant: 'primary', onClick: () => dom.fileInput.click() }),
      ),
    );
  } else {
    ...
    children.push(
      h('dl.summary', {}, [
        h('dt', { text: 'ファイル' }),
        h('dd', { text: `${state.media.name}（${formatBytes(state.media.size)}）` }),
        h('dt', { text: '長さ' }),
        h('dd', { text: `${formatTime(state.media.duration, { ms: true })}` }),
        h('dt', { text: '解像度 / fps' }),
        h('dd', { text: `${state.media.width}×${state.media.height} / ${state.media.fps} fps` }),
        h('dt', { text: '音声' }),
        h('dd', { text: state.media.hasAudio ? 'あり' : 'なし（映像のみモード）' }),
        h('dt', { text: '解析' }),
        h('dd', {
          text: a.done
            ? `シーン ${a.scenes.length} / 発話区間 ${a.speech.length} / フレーム ${a.frames.length}`
            : '未解析',
        }),
        h('dt', { text: 'ラウドネス' }),
        h('dd', {
          text: a.done && state.media.hasAudio
            ? `${a.loudness.integratedDb.toFixed(1)} dB（ピーク ${a.loudness.peakDb.toFixed(1)} dB）`
            : '—',
        }),
      ]),
      h('div.toolbar', {}, [
        button('別の素材を選ぶ', { onClick: () => dom.fileInput.click() }),
        button(a.done ? '再解析する' : '解析を実行', { variant: 'primary', onClick: () => runAnalysis() }),
        button('おまかせ自動処理', { variant: 'primary', onClick: () => runAutoPipeline() }),
      ]),
    );
    for (const warning of state.analysis.warnings) children.push(infoBox(warning));
  }
...
function renderEditPanel() {
  const state = store.getState();
  if (!state.media) {
    editPanelHost.replaceChildren(emptyState('素材が必要です', '先に動画を読み込んでください。'));
    return;
  }
  ...
  const clipList = h('div.clip-list', { attrs: { role: 'list', 'aria-label': 'クリップ一覧' } },
    state.clips.length
      ? state.clips.slice(0, 200).map((clip, index) =>
        h('div.clip-item', { attrs: { role: 'listitem' } }, [
          h('span.clip-item__label', {
            text: `#${index + 1} ${formatTime(clip.start, { ms: true })} → ${formatTime(clip.end, { ms: true })}${
          ...
          button('▶', {
            onClick: () => seek(sourceToTimelineSafe(clip.start).timelineTime),
  ...
  editPanelHost.replaceChildren(
    ...
    state.analysis.speech.length === 0 && state.analysis.done
      ? infoBox('発話区間が検出されていないため、自動カットは素材全体を 1 クリップとして扱います。')
      : h('div'),
...
function applyAutoCut() {
  ...
  const clips = buildCutPlan(state.analysis.speech, state.media.duration, state.autoEdit);
  store.commit((draft) => {
    draft.clips = clips;
    draft.autoEdit.enabled = true;
  }, { label: '自動カットを適用' });
  const summary = editSummary(clips, state.media.duration);
...
function resetClips() {
  ...
  if (!state.media) return;
  store.commit((draft) => {
    draft.clips = fullClip(draft.media.duration);
    draft.autoEdit.enabled = false;
  }, { label: 'クリップをリセット' });
  toast('クリップを素材全体に戻しました。', { type: 'success' });
}
...
function splitAtPlayhead() {
  ...
  store.commit((draft) => {
    draft.clips = sanitizeClips(splitAt(draft.clips, mapped.sourceTime), draft.media?.duration || 0);
  }, { label: 'クリップを分割' });
  if (store.getState().clips.length === before) {
    // 再生位置がクリップの端（または削除区間）にある場合は分割点を作れない
...
function renderAudioPanel() {
  ...
  const plan = buildAudioPlan(cfg, state.analysis.loudness, [], total);
  ...
  audioPanelHost.replaceChildren(
    state.media?.hasAudio === false
      ? infoBox('この素材には解析可能な音声トラックがありません。BGM の追加とフェードのみ利用できます。')
      : h('div'),
    h('div.controls', {}, [
...
async function handleFile(file) {
  if (!file) return;
  if (!file.type.startsWith('video/') && !/\.(mp4|webm|mov|mkv|ogv)$/i.test(file.name)) {
    toast('動画ファイルを選択してください（MP4 / WebM / MOV）。', { type: 'error' });
    return;
  }
  if (file.size > MAX_RECOMMENDED_BYTES) {
    const ok = await confirmDialog(
      `ファイルサイズが ${formatBytes(file.size)} と大きいため、解析に時間がかかり動作が不安定になる場合があります。続行しますか？`,
      { confirmLabel: '続行' },
    );
    if (!ok) return;
  }

  const progress = loading('素材を読み込み中…');
  ...
  try {
    ...
    store.replace(
      {
        ...store.getState(),
        name: file.name.replace(/\.[^.]+$/, '') || '無題のプロジェクト',
        media: meta,
        analysis: emptyAnalysis(),
        clips: fullClip(meta.duration),
        subtitles: [],
        thumbnail: { ...store.getState().thumbnail, sourceTime: Math.min(1, meta.duration / 2) },
...
async function runAnalysis({ askAfter = false } = {}) {
  ...
  try {
    const result = await analyzeMedia({
      ...
      onStage: ({ name, ratio }) => {
        progress.setLabel(`${name}（${Math.round(ratio * 100)}%）`);
        progress.setRatio(ratio);
      },
    });
    media.monoSamples = result.monoSamples;
    media.sampleRate = result.sampleRate;

    store.commit((draft) => {
      draft.analysis = result.analysis;
      draft.media.fps = result.fps;
      draft.media.hasAudio = result.hasAudio;
      if (!draft.clips.length) draft.clips = fullClip(draft.media.duration);
    }, { label: '解析' });

    await media.player?.seek(0);
    timeline.refresh();
    toast(
      `解析が完了しました：シーン ${result.analysis.scenes.length} 件 / 発話区間 ${result.analysis.speech.length} 件`,
      { type: 'success' },
    );
    announce('解析が完了しました。');
    renderMediaPanel();
    ...
    if (error.name === 'AbortError') {
      renderMediaPanel(infoBox('解析をキャンセルしました。'));
    } else {
...
async function offerAutoPipeline() {
  ...
  const cutCheck = h('input', { type: 'checkbox', checked: state.analysis.speech.length > 0, id: 'auto-cut' });
  ...
  const content = h('div', {}, [
    ...
    h('ul.check-list', {}, [
      ...
      h('li', {}, [cutCheck, h('label', { attrs: { for: 'auto-cut' }, text: '無音を自動カット' })]),
...
async function runAutoSubtitles({ silent = false } = {}) {
  ...
  const speech = state.analysis.speech;
  if (!speech.length) {
    if (state.media?.hasAudio === false) {
      toast('音声トラックがないため字幕を自動生成できません。手動で追加してください。', { type: 'error' });
    } else {
      toast('発話区間を検出できませんでした。手動で字幕を追加してください。', { type: 'error' });
    }
    return;
  }
  ...
  if (state.stt.providerId === 'remote') {
    await runRemoteStt(speech);
    return;
  }

  const result = transcribeWithVad(speech);
  store.commit((draft) => {
    draft.subtitles = normalizeCues(result.cues, draft.media?.duration || 0);
  }, { label: '字幕を自動生成' });
  toast(result.notice, { type: 'success' });
  announce(result.notice);
}

async function runRemoteStt(speech) {
  const state = store.getState();
  if (!media.monoSamples || !media.sampleRate) {
    toast('音声データがありません。再解析してください。', { type: 'error' });
    return;
  }
  ...
  const resampled = resample(media.monoSamples, media.sampleRate, targetRate);
  const wav = encodeWav(resampled, targetRate);
  const sizeMb = wav.byteLength / 1024 / 1024;
  if (sizeMb > 24) {
    const ok = await confirmDialog(
      `送信する音声が ${sizeMb.toFixed(1)} MB あります。多くの API は 25MB を上限としており失敗する可能性があります。続行しますか？`,
      { confirmLabel: '送信する' },
    );
    if (!ok) return;
  }
  ...
  const progress = loading(`音声を送信して文字起こし中…（${sizeMb.toFixed(1)} MB）`);
  ...
  try {
    ...
    if (response.errors?.length && !response.cues.length && !response.plainText) {
      renderMediaPanel(
        errorBox(
          `文字起こしに失敗しました：${response.errors[0]}`,
          button('タイムコードのみ生成する', {
            onClick: () => {
              const fallback = transcribeWithVad(speech);
              store.commit((draft) => {
                draft.subtitles = normalizeCues(fallback.cues, draft.media?.duration || 0);
              }, { label: '字幕を自動生成' });
              renderMediaPanel();
              toast(fallback.notice, { type: 'success' });
            },
          }),
        ),
      );
    ...
    const cues = response.cues.length ? response.cues : alignTextToSegments(response.plainText || '', speech);
    store.commit((draft) => {
      draft.subtitles = normalizeCues(cues, draft.media?.duration || 0);
    }, { label: '字幕を自動生成' });
    renderMediaPanel();
    toast(`文字起こしが完了しました（${cues.length} 件）。`, { type: 'success' });
    announce(`文字起こしが完了しました。${cues.length} 件の字幕を生成しました。`);
  } catch (error) {
    if (error.name === 'AbortError') {
...
function resizePreviewCanvas() {
  const state = store.getState();
  const width = state.media?.width || 1280;
  const height = state.media?.height || 720;
  const maxWidth = 1280;
  const scale = Math.min(1, maxWidth / width);
  dom.preview.width = Math.round(width * scale);
  dom.preview.height = Math.round(height * scale);
}

function sourceToTimelineSafe(sourceTime) {
  const state = store.getState();
  const laid = layoutClips(state.clips);
