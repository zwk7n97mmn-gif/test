...
| styling     | 色（インラインCSS・<font>）で意味を表している（環境によっては消える） |
...
FONT_TAG = re.compile(r"<font\b", re.I)
# color / background / border-color など、見た目の色を決めるもの
COLOR_PROP = re.compile(r"(?:^|[;\s])(?:color|background|background-color|border-color)\s*:", re.I)
...
class Linter:
    ...
    def check_styling(self) -> None:
        """色で意味を表さない決まり。

        Gitea や GitHub はインラインCSSを落とすことがあり、落ちると
        「赤 ＝ 重要」が**ただの黒い文字**になって意味ごと消える。
        目立たせたいものは ⚠️ 🚨 と **太字** で表す。
        """
        problems = []
        for page in self.pages:
            rel = page.relative_to(self.root)
            for number, line in enumerate(mask_code(page.read_text(encoding="utf-8")).splitlines(), 1):
                if STYLE_BLOCK.search(line):
                    problems.append(f"{rel}:{number} … <style> は消える。⚠️ と **太字** で表す")
                elif FONT_TAG.search(line):
                    problems.append(f"{rel}:{number} … <font> は消える。⚠️ と **太字** で表す")
                elif any(COLOR_PROP.search(value) for value in STYLE_ATTR.findall(line)):
                    problems.append(f"{rel}:{number} … 色は消える。⚠️ と **太字** で表す")
        self.report(not problems, f"色に意味を持たせていない（{len(problems)} 件）", problems)
