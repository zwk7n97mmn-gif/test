...
このアカウントは 1 日 2 回 = 月約 60 投稿なので、無料枠に十分収まります。
（枠は X 側の都合で変わることがあります。上限に達した場合は投稿が
「失敗」ではなく「見送り」として記録され、Bluesky 側は通常どおり投稿されます）
...
セットアップはここまでです。以降に必要になる資料はこちらにあります。

| 知りたいこと | ファイル |
|---|---|
| 手動実行・失敗したときの対処・投稿時刻の変更・止め方 | [OPERATIONS.md](OPERATIONS.md) |
| 中で何が起きているか | [ARCHITECTURE.md](ARCHITECTURE.md) |
| 豆知識の書き方 | [EDITORIAL.md](EDITORIAL.md) |

分からないことがあれば、Issue を立てて `help-AI` ラベルを付けると
AI が資料を読んで答えます（[AI-ASSISTANT.md](AI-ASSISTANT.md)）。
